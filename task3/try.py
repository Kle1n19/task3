...
def get_track_markets(track_name):
    ...
    response = get(f'https://api.spotify.com/v1/search?q={query}&type=track&limit=1', headers=get_auth_header(get_token()))
    search_results = response.json()
    if search_results['tracks']['total'] == 0:
        return None
    else:
        track_id = search_results['tracks']['items'][0]['id']
        response = get(f'https://api.spotify.com/v1/tracks/{track_id}', headers=get_auth_header(get_token()))
        track_info = response.json()
        return track_info['available_markets']

def get_count_by_artist_name(name):
    result = search_for_artist(get_token(),name)
    artist_id = result['id']
    songs = get_songs_by_artist(get_token(),artist_id)
...
def search():
    if request.method == 'POST':
        ...
        map = gen_map(get_coords(get_count_by_artist_name(name)))
